...
async function handleFile(file){
  ...
  try{
    ...
    moveEditorTo('pro');
    buildEditor();
    // === GC builder: mark detected sections + render ===
    if (typeof gcSections !== 'undefined') {
      const has = (v) => Array.isArray(v) ? v.length > 0 : (v && typeof v === 'object' ? Object.keys(v).length > 0 : !!v);
      const dmap = {
        personal:       has(R.personal) || has(R.name) || has(R.email),
        certifications: has(R.certifications),
        education:      has(R.education),
        skills:         has(R.skills),
        summary:        has(R.summary),
        projects:       has(R.projects),
        experience:     has(R.experience),
        awards:         has(R.awards) || has(R.achievements),
      };
      gcSections.forEach(s => { s.detected = !!dmap[s.id]; });
      const mount = document.getElementById('builder-root');
      if (mount) renderGcBuilder(mount);
    }
...
/* ================= GC Section Builder (redesigned panel) ================= */
const GC_ICONS = {
  drag:  '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="6" r="1"/><circle cx="9" cy="12" r="1"/><circle cx="9" cy="18" r="1"/><circle cx="15" cy="6" r="1"/><circle cx="15" cy="12" r="1"/><circle cx="15" cy="18" r="1"/></svg>',
  ...
  check: '<svg class="gc-check" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><polyline points="8 12 11 15 16 9"/></svg>',
  plus:  '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>',
  ai:    '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l2 6 6 2-6 2-2 6-2-6-6-2 6-2z"/></svg>',
};
let gcSections = [
  { id:'personal',       name:'Personal details', group:'header', detected:false, inResume:true },
  { id:'certifications', name:'Certifications',   group:'header', detected:false, inResume:true, hasAdd:true, tag:'HEADER CREDENTIALS' },
  { id:'education',      name:'Education',        group:'header', detected:false, inResume:true, hasAdd:true, tag:'HEADER CREDENTIALS' },
  { id:'skills',         name:'Skills',           group:'body',   detected:false, inResume:true },
  { id:'summary',        name:'Summary',          group:'body',   detected:false, inResume:true, hasAI:true },
  { id:'projects',       name:'Projects',         group:'body',   detected:false, inResume:false, hasAdd:true },
  { id:'experience',     name:'Experience',       group:'body',   detected:false, inResume:true, hasAdd:true },
  { id:'awards',         name:'Awards',           group:'body',   detected:false, inResume:false, hasAdd:true },
];
let gcActiveId = 'summary';

function gcSideItem(s){
  const a = s.id === gcActiveId ? 'active' : '';
  return `<li><button class="gc-side-item ${a}" data-id="${s.id}"><span>${s.name}</span>${s.detected?GC_ICONS.check:''}</button></li>`;
}
function gcBar(s){
  const a = s.id === gcActiveId ? 'active' : '';
  const on = s.inResume ? 'on' : '';
  return `
  <div class="gc-bar ${a}" data-id="${s.id}">
    <button class="gc-drag">${GC_ICONS.drag}</button>
    <div class="gc-title-wrap">
      <h3 class="gc-title">${s.name}</h3>
      ${s.hasAdd?`<button class="gc-badge gc-badge-add">${GC_ICONS.plus} Add</button>`:''}
      ${s.hasAI?`<button class="gc-badge gc-badge-ai">${GC_ICONS.ai} AI enhance</button>`:''}
      ${s.detected?`<span class="gc-detected">${GC_ICONS.check} Detected</span>`:''}
      ${s.tag?`<span class="gc-badge gc-badge-tag">${s.tag}</span>`:''}
    </div>
    <div class="gc-right">
      <button class="gc-toggle ${on}" data-toggle="${s.id}"></button>
      <span class="gc-toggle-label ${on}">In resume</span>
      <button class="gc-chev ${a}">${GC_ICONS.chev}</button>
    </div>
  </div>`;
}
function renderGcBuilder(mount){
  const head = gcSections.filter(s=>s.group==='header').map(gcBar).join('');
  const body = gcSections.filter(s=>s.group==='body').map(gcBar).join('');
  mount.innerHTML = `
    <div class="gc-wrap"><div class="gc-grid">
      <aside class="gc-side">
        <div class="gc-side-h">@sections</div>
        <ul>${gcSections.map(gcSideItem).join('')}</ul>
      </aside>
      <main>
        <div class="gc-group"><span class="gc-group-label">Header credentials</span><span class="gc-group-tag">Print under your name</span><div class="gc-group-line"></div></div>
        ${head}
        <div class="gc-group"><span class="gc-group-label">Resume body</span><span class="gc-group-tag">Drag to arrange print order</span><div class="gc-group-line"></div></div>
        ${body}
      </main>
    </div></div>`;
  mount.querySelectorAll('[data-id]').forEach(el=>{
    el.addEventListener('click', e=>{
      if(e.target.closest('[data-toggle]')) return;
      gcActiveId = el.dataset.id;
      renderGcBuilder(mount);
    });
  });
  mount.querySelectorAll('[data-toggle]').forEach(el=>{
    el.addEventListener('click', e=>{
      e.stopPropagation();
      const s = gcSections.find(x=>x.id===el.dataset.toggle);
      s.inResume = !s.inResume;
      renderGcBuilder(mount);
    });
  });
}
// Auto-mount on load if the div exists
document.addEventListener('DOMContentLoaded', ()=>{
  const m = document.getElementById('builder-root');
  if (m) renderGcBuilder(m);
});
